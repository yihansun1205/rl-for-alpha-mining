...
Supplement from self-learning notes: the AlphaGen-style environment is effectively deterministic at the transition level (\(s_{t+1}=s_t\oplus a_t\)); most uncertainty comes from policy sampling and noisy evaluator outputs rather than environment dynamics. The notes also emphasize that AlphaGen’s non-stationarity is mostly reward-side, because pool updates change the marginal value of newly generated formulas across training. This helps explain why action masking, invalid-expression penalties, and pool update rules can matter as much as the PPO optimizer itself.
...
Supplement from self-learning notes: for RiskMiner, the notes make the reward-dense mechanism explicit as
\[
R_{\text{inter}}=\mathrm{IC}(f)-\lambda \frac{1}{k}\sum_{i=1}^{k}\mathrm{mutIC}(f,f_i),\qquad
R_{\text{end}}=\mathrm{IC}\!\left(\sum_i \omega_i f_i(X_t),r_{t+1}\right),
\]
highlighting that diversity control is built into both intermediate and terminal evaluation. They also clarify a structural point often missed in summaries: MCTS statistics are naturally edge-based (\(Q(s,a),N(s,a)\)) on the RPN-prefix tree, which is different from one-pass token append sampling. For Alpha2, the notes further sharpen that “program construction + register semantics + dimension-aware pruning” is not just a representation change; it is an early-error-elimination mechanism that converts many invalid-terminal penalties into pre-expansion pruning.
...
Supplement from self-learning notes: QuantFactor REINFORCE can be written as a baseline-adjusted Monte Carlo update
\[
\nabla_\theta J(\theta)\approx \sum_t \nabla_\theta \log \pi_\theta(a_t\mid s_t)\,\bigl(r(a_{1:T})-r(\bar a_{1:T})\bigr),
\]
where \(r(\bar a_{1:T})\) is the greedy-decoded trajectory reward used as a control variate. The notes report that removing either the greedy baseline or IR shaping degrades performance, supporting the claim that variance control and steadiness-aware reward are complementary. For TLRS, the notes also clarify that shaped rewards are centered online to control drift, which is practically important when expert-subsequence matching is dense but noisy.
...
Supplement from self-learning notes: the AlphaQCM notes make three implementation-relevant points explicit. First, MDP dynamics remain AlphaGen-like (RPN prefix state, deterministic append transition, masked legality), so the main change is in value learning rather than environment design. Second, the reward remains terminal and pool-aware, including invalid-formula penalties and pool-marginal IC improvements. Third, the action score is not mean-only selection but an uncertainty-aware blend \(Q+\lambda\sigma_{\mathrm{QCM}}\), where \(\sigma_{\mathrm{QCM}}\) is estimated from learned quantiles rather than naive sample variance, targeting robustness under non-stationary pool updates.
...
Supplement from self-learning notes: AlphaForge is better interpreted as a surrogate-guided symbolic optimization framework than as a strict MDP-RL algorithm. The notes highlight that stage-1 generation is driven by a differentiable generator-predictor loop (with rule masking and relaxed discrete selection), while stage-2 performs daily factor timing using recent IC/ICIR signals and rolling linear recombination. Reported ablations in the notes also indicate that dynamic combination materially outperforms static weighting, and larger factor-zoo size is not always better, which reinforces the deployment-side bottleneck argument.
...
Recent 2026 work can be viewed as extensions rather than replacements of the 2023–2025 core line. **AlphaSAGE (ICLR 2026)** emphasizes structure-aware diverse generation by combining AST-based encoding (RGCN-style structural encoding) with GFlowNet-style sampling and multi-signal rewards (predictive power, structure-behavior alignment, novelty). Its main value is reducing mode collapse while keeping symbolic interpretability; practically, it shifts the objective from “optimize one best formula” to “learn a sampling distribution over many high-reward formulas.” **AlphaAgentEvo (ICLR 2026)** reframes alpha mining as multi-turn policy evolution: the agent repeatedly refines seed formulas through tool feedback under hierarchical rewards. Its main value is improving evolution efficiency and robustness in longer interaction horizons.

Supplement from self-learning notes: AlphaSAGE’s optimization target is explicitly distributional at the formula level,
\[
P_\theta(\alpha)\propto R(\alpha),\qquad
R(\alpha,T)=R_{IC}(\alpha)+\lambda(T)R_{SA}(\alpha)+\eta(T)R_{NOV}(\alpha),
\]
with terminal alpha reward decomposed into predictive quality, structure-behavior alignment, and novelty against known high-quality formulas. This differs from AlphaGen-style marginal pool improvement objectives and makes diversity an intrinsic target rather than a post-hoc filter.

The notes further clarify two implementation-level points: state is a partial AST (not only an RPN prefix), and training uses GFlowNet Trajectory Balance plus entropy regularization, which explicitly trades off reward-proportional sampling and exploration stability. Reported ablations in the notes indicate that removing structure-aware or novelty components degrades ranking robustness, while overly large novelty weight can hurt raw IC, illustrating a real scalarization tradeoff.
...
| AlphaSAGE | ICLR 2026 | Reward-proportional formula distribution learning | AST + RGCN | GFlowNet with Trajectory Balance | IC + structure-aware + novelty (+ entropy regularization) | Partly (novelty against known pool) | Yes (with dynamic combination stage) | Makes diversity-first generation a first-class optimization target | Reward scalarization sensitivity; cost/turnover realism still limited |

### 5.3 Comparative Discussion

A concise way to compare representative work is by the role RL plays in each framework. AlphaGen mainly uses RL as a direct symbolic generator; RiskMiner and Alpha2 push RL toward structured search control; QuantFactor REINFORCE and TLRS treat learning-signal quality as the primary bottleneck; AlphaQCM introduces uncertainty-aware exploration; AlphaForge explicitly couples discovery and deployment; AlphaSAGE reframes generation as reward-proportional distribution learning with explicit diversity pressure.
...
Recent quantitative gains are encouraging but should be read with protocol caution: AlphaQCM reports stronger IC on large universes (e.g., 8.49%, 9.55%, 9.16% in its setting), QuantFactor REINFORCE reports about 3.83% correlation improvement, and TLRS reports about 9.29% RankIC improvement over potential-based shaping baselines. Notes-based synthesis adds two practical observations: AlphaForge’s dynamic-combination ablation suggests deployment logic can drive gains comparable to generator-side changes, and RiskMiner/Alpha2-style methods show that legality-aware tree/program search can convert many late-stage invalid-expression failures into early pruning.
